...
def refreshData():
    ...
    data = pd.concat([data_GLOB, data_US], sort=True)
    mask = (data['date'] >= start_date)
    filtered_data = data.loc[mask]
    filtered_data.to_pickle(fileNamePickle)
...
app.layout = html.Div(
    ...
    children=[
        html.H1('CS6440 Mini Project II: COVID-19 Case Dashboard'),
        html.H3('Author: Chong Dang'),
...
def filtered_data(country, state):
    ...
    newCases = data.select_dtypes(include='Int64').fillna(0)
...
def update_plots(country, state, metrics, n):
    ...
    barchart_new = barchart(data, metrics, prefix="New", yaxisTitle="New Cases")
    barchart_cum = barchart(data, metrics, prefix="Cum", yaxisTitle="Cumulative Cases")
    return barchart_new, barchart_cum

server = app.server

if __name__ == '__main__':
    app.run_server(host="0.0.0.0", port=6440)
